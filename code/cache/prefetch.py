...
void lruTwoListClass::init(uint64_t numCL) {		/* lt->init(cpara.set_associativity); */
	activeBound = numCL / 3;
	activeBound = 0;
}
...
void lruTwoListClass::updateWhenHit(uint64_t lba, bool& falsehit) {
	...
	if (initer == inactive.begin() && *initer != lba) {
		//cout << "Wrong" << endl;
		falsehit = 1;
		return;		/* inactive中也没找到 */
	}

	inactive.erase(initer);
	if (activeBound == 0) inactive.push_back(lba);		/* 找到了 但没开放active 就移至inactive队头 */

	if (activeBound!=0 && active.size() == activeBound) {		/* 找到了 但active满了 就进行转移 */
...
void lfuHeap::add(uint64_t addr) {
	lfuNode n{ addr };		/* (addr, count) */
	h.push_back(n);		/* vector<lfuNode> h; */
	m.emplace(addr, h.size() - 1);		/* map<uint64_t, uint64_t> m; */
	...
	while (1) {
		...
		if (h[pi].count > h[ci].count) {	/* 貌似索引小的就count小 一步步往前 */
...
void lfuHeap::update(uint64_t addr) {
	...
	while (1) {
		uint64_t lci{ 2 * ci + 1 }, rci{ 2 * ci + 2 };		/* 貌似建立了个平衡二叉树 */
...
namespace SSD_Components {
	...
	void dram_subsystem::initDRAM() {
		...
		for (uint64_t i = 0; i < cpara.cache_portion_size / cpara.ssd_page_size / cpara.set_associativity; i++) {
			...
			/* 就是总共分为cache_portion_size / ssd_page_size / set_associativity个组，每个组set_associativity个cache line，貌似每个组内遵循对应算法 */
	...
	void dram_subsystem::process_cache_hit(bool rw, uint64_t lba, bool& falsehit) {
		/* cache hit下的缓存队列调整和更新 */

		if (!this->isCacheHit(lba)) {	/* 貌似一般情况下不会执行 */
			falsehit = 1;
			return;		/* 未hit */
		}
		...
		if (!cpara.mix_mode) {		/* 目前不会执行 */
			...
			cache_page_addr = (*dram_mapping)[lba];		/* LBA, cache line index */
		}



		uint64_t cache_index{ get_cache_index(lba) };	/* return lba % (cpara.cache_portion_size / cpara.ssd_page_size / cpara.set_associativity); */
		...
		if (cache_page_addr < cpara.cache_portion_size / cpara.ssd_page_size) { // check if the cache hit is at cache portion or prefetch portion
			...
			if (cpara.cpolicy == cachepolicy::lru2) {
				...
				*next_eviction_candidate = lrfucachedlba->getCandidate();
				...
				(*all_cflrucachedlba)[cache_index]->modify(lba, !rw);	/* (uint64_t pn, bool dirty) 如果是写 那就会变成脏页 */
			}
	...
	void dram_subsystem::process_miss_data_ready_new(bool rw, uint64_t lba, list<uint64_t>* flush_lba, uint64_t simtime, set<uint64_t>* prefetched_lba, set<uint64_t>&taggedAddr, set<uint64_t>& prefetch_pollution_tracker,  set<uint64_t> not_finished) {
		...
		if (temp_dram_mapping->count(lba) > 0) {		/* hit了 */
			bool falsehit{ 0 };
		...
		if (temp_freeCL->empty()) {
			...
			if (temp_dirtyCL->count(cl) > 0) {		/* 如果是脏页就需要flush 在这个函数外的调用函数中处理 */
				flush_lba->push_back(evict_lba_base_addr);
				//evictf.of << evict_lba_base_addr << endl;
